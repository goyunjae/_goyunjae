(function () {
  ...
  function projectHome() {
    ...
    document.body.insertAdjacentHTML("beforeend", '<section class="project-home" hidden><div class="project-home-head"><strong>My projects</strong><button type="button" id="homeClose">Open editor</button></div><aside class="project-nav"><button class="active">My projects</button><button>Drafts</button><button>Maps</button><button>Published projects</button><button>All projects</button></aside><main class="project-grid"><button type="button" class="project-card" data-type="PIE"><span class="project-thumb sunburst"></span><b>Income per country</b></button><button type="button" class="project-card" data-type="BAR"><span class="project-thumb bar"></span><b>Audience Growth</b></button><button type="button" class="project-card" data-type=""><span class="project-thumb folder"></span><b>Growth</b></button></main></section>');
    document.querySelector("#homeClose")?.addEventListener("click", () => showHome(false));
    document.querySelectorAll(".project-card").forEach((card) => card.addEventListener("click", () => { showHome(false); if (card.dataset.type) choose(card.dataset.type); }));
  }
  ...
  function removeTemplateGallery() {
    document.querySelectorAll(".template-gallery-board").forEach((node) => node.remove());
  }
  ...
  function observe() {
    ...
    if (!grid) return;
    new MutationObserver(removeTemplateGallery).observe(grid, { childList: true });
  }
  function storyModal() {
  ...
  function openStory() {
    ...
    const modal = document.querySelector(".studio-modal");
    modal.querySelector(".modal-body").innerHTML = '<h2>Story preview</h2><div class="story-preview"><aside><b>Your Feed</b><span>Home</span><span>Search</span><span>Explore</span><span>Messages</span></aside><article><div class="story-chart">' + title + '</div><p><b>G2 Studio</b> This chart is ready to share as a report story.</p></article></div>';
    modal.showModal();
  }
  function style() {
